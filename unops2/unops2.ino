...
void arm_control(){
  ...
  if (isFlagIn) flag_angle = constrain(flag_angle + 0.2, 0, 180);
  if (isFlagDe) flag_angle = constrain(flag_angle - 0.2, 0, 180);

  centerServo.write((int)center);
  armServo1.write((int)arm1Servo);
  armServo2.write((int)arm2Servo);
  flag.write((int)flag_angle);
}
